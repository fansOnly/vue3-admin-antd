...
Mock.mock(/\/product\/category\/index/, /get|post/i, getCategoryIndex);

/**
 * 获取文章分类详情
 * @param {String} id
 */
const getCategoryDetail = config => {
...
Mock.mock(/\/roduct\/category\/detail/, /get|post/i, getCategoryDetail);
...
Mock.mock(/\/product\/category\/add/, /get|post/i, addCategory);
...
Mock.mock(/\/product\/category\/update/, /get|post/i, updateCategory);

/**
 * 删除文章分类
 * @param {Array} ids
 */
const deleteCategory = config => {
...
Mock.mock(/\/product\/category\/delete/, /get|post/i, deleteCategory);
...
Mock.mock(/\/product\/category\/tree/, /get|post/i, getCategoryTree);
...
Mock.mock(/\/product\/list/, /get|post/i, getProductList);

/**
 * 获取文章详情
 * @param {String} id
 */
const getProductDetail = config => {
    const params = getUrlParams(config.url);
    const { id } = params;
    const success = id;
...
Mock.mock(/\/product\/detail/, /get|post/i, getProductDetail);
...
Mock.mock(/\/product\/add/, /get|post/i, addProduct);
...
Mock.mock(/\/product\/update/, /get|post/i, updateProduct);

/**
 * 获取文章回收站列表
 */
const getProductRecycleList = config => {
    const params = getUrlParams(config.url);
    const { page, pageSize, state, delete_time } = params;
    const randomLen = 170;
    const len = randomLen - pageSize * (page - 1) < pageSize ? randomLen - pageSize * (page - 1) : pageSize;
...
Mock.mock(/\/product\/recycle/, /get|post/i, getProductRecycleList);

/**
 * 删除文章
 * @param {Array} ids
 */
const deleteProduct = config => {
    const params = JSON.parse(config.body);
    const { ids } = params;
    const success = ids.length;
...
Mock.mock(/\/product\/delete/, /get|post/i, deleteProduct);

/**
 * 还原文章
 * @param {Array} ids
 */
const restoreProduct = config => {
    const params = JSON.parse(config.body);
    const { ids } = params;
    const success = ids.length;
...
Mock.mock(/\/product\/restore/, /get|post/i, restoreProduct);

/**
 * 彻底清除文章
 * @param {Array} ids
 */
const clearProduct = config => {
    const params = JSON.parse(config.body);
    const { ids } = params;
    const success = ids.length;
...
Mock.mock(/\/product\/clear/, /get|post/i, clearProduct);
