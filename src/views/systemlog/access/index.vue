<template>
    ...
    >
        ...
        <template #filterBeforeSlot="{ filterValues }">
            <a-col v-if="filterList.FILTER_MEMBER_NICK_NAME" :span="6">
                    <a-form-model-item :label="$t('GLOBAL.NICK_NAME')">
                        <a-input v-model="filterValues.fmembernickname" :placeholder="$t('GLOBAL.OPTION_INPUT') + $t('GLOBAL.NICK_NAME')" />
                    </a-form-model-item>
                </a-col>
        </template>
        <!-- 渲染数据 -->
...
<script>
    ...
    import { getAccesslogList as getDataList, deleteAccesslog as deleteDataList } from '@/api/systemlog'
