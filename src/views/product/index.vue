<template>
    ...
    >
        ...
        <template #filterBeforeSlot="{ filterValues }">
            <a-col v-if="filterList.FILTER_CATEGORY_ID" :span="6">
                <a-form-model-item :label="$t('GLOBAL.DATA_SECTION')">
                    <a-tree-select
                        style="width: 100%"
                        v-model="filterValues.fcatogoryid"
        ...
        <template v-slot:tableSlot>
            <a-table row-key="id" :loading="loading" :columns="columnList" :data-source="dataList" :pagination="pagination" :row-selection="rowSelection" bordered @change="handleTableChange" >
                <template v-slot:thumbSlot="action" >
                    ...
                    <a v-else-if="action[0].state == 1" href="javascript:;"><img :src="getImgAbsPath(action[0].path)" style="width:30px;height:30px;" :alt="action[0].name" @click="handlePhotoPreview(action[0])"></a>
...
<script>
    ...
    import { getProductList as getDataList, deleteProduct as deleteDataList, getCategoryTree as getTree } from '@/api/product'
	...
	export default {
		...
		created () {
            ...
            this.filterList.FILTER_CATEGORY_ID && this.getTreeFn();
			this.getDataListFn();
		},
		methods: {
