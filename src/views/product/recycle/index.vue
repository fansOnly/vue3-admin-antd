        ...
        <template v-slot:tableSlot>
            <a-table row-key="id" :loading="loading" :columns="columnList" :data-source="dataList" :pagination="pagination" :row-selection="rowSelection" bordered @change="handleTableChange" >
                ...
                <span slot="actionSlot" slot-scope="action, record">
                    ...
                    <span>&nbsp;</span>
                    <a-popconfirm :title="$t('GLOBAL.OPTION_DELETE_CONFIRM')" @confirm="() => clearItem(record.id)">
...
import { getProductRecycleList as getDataList, restoreProduct as restoreDataList, clearProduct as deleteDataList, getCategoryTree as getTree, clearAll } from '@/api/product'
