...
import { getBannerClassList as getDataList, deleteBannerClass as deleteData } from '@/api/banner'
...
export default {
    ...
    setup() {
            ...
            const handleClickItem = (type, id) => {
                if (type === 'add') {
					router.push({name: 'bannerClassAdd'})
                } else if (type === 'edit') {
					router.push({name: 'bannerClassEdit', query: {id}})
                } else if (type === 'delete') {
                    apiDeleteData([id])
                } else {
