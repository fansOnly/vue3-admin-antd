    ...
    >
        ...
        <template v-slot:tableSlot>
            <a-table row-key="id" :loading="loading" :columns="columnList" :data-source="dataList" :pagination="pagination" :row-selection="rowSelection" bordered @change="handleTableChange" >
                <template v-slot:avatarSlot="action" >
                    <span v-if="!action">{{$t('GLOBAL.TEXT_EMPTY')}}</span>
...
<script>
    ...
    import { getMemberList as getDataList, deleteMember as deleteDataList } from '@/api/member'
    ...
    export default {
        ...
        methods: {
            ...
            routePage (action, editId) {
                if (action == 'add') {
					this.$router.push({name: 'memberAdd'});
                } else if (action == 'edit') {
					this.$router.push({name: 'memberEdit', params: {id: editId}});
                } else {
