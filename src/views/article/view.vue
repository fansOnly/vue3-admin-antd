				...
				<a-col :span="7" :offset="1">
					...
					<a-form-model-item v-bind="formRightLayout" :label="$t('GLOBAL.TAG')">
						<a-select v-model="viewData.tags" mode="multiple" allow-clear :placeholder="$t('GLOBAL.OPTION_SELECT') + $t('GLOBAL.TAG')">
							<a-select-option v-for="(item, index) in ARTICLE_TAGS" :key="index" :value="item">{{item}}</a-select-option>
						</a-select>
					</a-form-model-item>
...
import { getArticleDetail as getData, addArticle as addData, updateArticle as updateData, getSectionTree as getTree } from '@/api/article'
import { INFO_STATUS, ARTICLE_TAGS } from 'config/setting'
...
export default {
	...
	methods: {
		handleUpload(data, tag) {
			// console.log('_savedIds:', data.savedIds,'_deletedIds:', data.deletedIds);
		...
		async addDataFn (params) {
			...
			if (res.code == '200') {
				this.$message.success(res.msg, 2, () => {
					this.submitDisabled = false;
					this.$router.replace({name: 'articleEdit', params:{id: res.data}});
				})
			} else {
				this.$message.error(res.msg, () => {
					this.submitDisabled = false;
